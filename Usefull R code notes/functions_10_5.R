...
second_function <- function(x=10){return(x+3)}
second_function()


clean_bird_function('../Data/Bird_Measurements.csv')

source ('./')#takes an r script for input if you save a function as a .R file it can be run using this function


#turn cleaning script into a function that returns a data frame

clean_bird_function <- function(x){
  library(tidyverse)
  library(janitor)
  library(skimr)
  mass <- read.csv(x) %>% 
